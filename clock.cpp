...
Clock::Clock() :
  ...
  framerate(Gamedata::getInstance().getXmlInt("framerate")), 
  frames(0), 
  recentFrames(),
  recFrames(),
  maxFramesToAvg(Gamedata::getInstance().getXmlInt("frameCap")),
  tickSum(0),
  sumOfAllTicks(0),
...
Clock::Clock(const Clock& c) :
  ...
  frames(c.frames), recentFrames(c.recentFrames), recFrames(c.recFrames),
...
unsigned int Clock::capFrameRate() const { 
  ...
  unsigned int delay = std::max(0.0,1000.0/framerate+0.5 - ticks);
  SDL_Delay( delay );
  return delay;
}
...
int Clock::getAvgFps() const { 
  if ( getSeconds() > 0 && recFrames.size() > maxFramesToAvg/2) {
    return recFrames.size()/(tickSum/1000.0);  
  }
  if ( getTicks() > 10000 and getFrames() == 0 ) {
    throw std::string("Can't getFps if you don't increment the frames");
  }
  return 0;
}
...
Clock& Clock::operator++() { 
  if ( !paused ) {
    ++frames; 
    if ( recFrames.size() >= maxFramesToAvg ) { 
      tickSum -= recFrames.front();
      recFrames.pop(); 
    }
    recFrames.push(ticks);
    tickSum += ticks;
  }
