...
public:
  Drawable(const std::string& n, const Vector2f& pos, const Vector2f& vel): 
    name(n), position(pos), velocity(vel), move(0) {}

  Drawable(const Drawable& s) : 
    name(s.name), position(s.position), velocity(s.velocity), move(0)
    { }
  ...
  virtual void update(Uint32 ticks, float catAt) = 0;
  ...
  virtual void play(const int moveIn) { move = moveIn ; }
